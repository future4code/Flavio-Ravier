...
import { fetchProfileToSwipe,chooseProfile } from '../../actions/profiles'
...
export class SwipeScreen extends Component {
	...
	componentDidMount() {
		if (!this.props.profileToSwipe && this.props.getProfileToSwipe) {
			this.props.getProfileToSwipe() // verifica se existe uma imagem pra colocar, se não existir, 
			...
			<SwipeScreenWrapper>
				...
				<ContentWrapper>
					...
					{profileToSwipe ? <UserSwipeCard
						userToSwipe={profileToSwipe} /* this.props.profileToSwipe */
						animationDirection={currentAnimation}
					/> : (<Loader/>)}
					<ButtonsWrapper>  {/* aqui está a foto do personagem */}
...
const mapDispatchToProps = (dispatch) => {
	return {
		goToMatchScreen: () => dispatch(updateCurrentPage('MatchScreen')),  //trocar de página
		getProfileToSwipe: () => dispatch(fetchProfileToSwipe()),  //chama a função de colcar o perfil
